...
<td>操作</td>
			...
			<!--显示数据列表-->
			<tbody id="TableData" class="dataContainer" datakey="fileInfoList">

				<s:iterator value="#fileInfoList">
					<tr class="TableDetail1 template">
						<td><s:a action="fileInfo_list?parentId=%{id}">${fileName}</s:a>&nbsp;</td>
					...
					<td>${createDate}&nbsp;</td>
						...
						<td>3${description}&nbsp;</td>
						<td><s:a
								action="fileInfo_delete?id=%{id}&parentId=%{parent.id}"
								onclick="return window.confirm('这将删除所有的下级部门，您确定要删除吗？')">删除</s:a>
							<s:a action="fileInfo_editUI?id=%{id}">
						修改123
					</s:a> &nbsp;</td>
					</tr>
				</s:iterator>
		...
		<div id="TableTail">
			<div id="TableTail_inside">
				<s:a action="fileInfo_addUI?parentId=%{parentId}">
					<img
						src="${pageContext.request.contextPath}/style/images/createNew.png" />
				</s:a>
				<s:a action="fileInfo_list?parentId=%{#parent.parent.id}">
					<img
